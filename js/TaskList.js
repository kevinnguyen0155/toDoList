export class TaskList {
  ...
  showList(list) {
    ...
    if (list === this.toDoList) {
      ...
      document.getElementById("text-noToDo").style.display = "none"
      checkClassName = "far"
    } else {
      listContent = document.getElementById("done-list")
      document.getElementById("text-noDone").style.display = "none"
      checkClassName = "fas done"
    }
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      <i class="${checkClassName} fa-check-circle" name="${task.id}"></i>
      <i class="fa fa-trash-alt" name="${task.id}"></i>
    </div>`
      )
      this.setCheckButton(task.id)
    }
  }
